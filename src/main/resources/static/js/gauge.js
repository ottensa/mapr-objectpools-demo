var gauge = (function(){
	var _config = {
        minValue: -1, // The gauge minimum value.
        maxValue: 100, // The gauge maximum value.
        circleThickness: 0.05, // The outer circle thickness as a percentage of it's config.radius.
        circleFillGap: 0.05, // The size of the gap between the outer circle and wave circle as a percentage of the outer circles config.radius.
        circleColor: "#178BCA", // The color of the outer circle.
        waveHeight: 0.05, // The wave height as a percentage of the config.radius of the wave circle.
	...
	return {
		liquid: function(pct, cfg) {
			var config = Object.assign({}, _config, cfg);

			 var element = document.createElementNS(d3.ns.prefix.svg, "g");

			 var gauge = d3.select(element);
			 gauge.classed("gauge", true);

			 var fillPercent = Math.max(config.minValue, Math.min(config.maxValue, pct))/config.maxValue;
		    ...
		    var textPixels = (config.textSize*config.radius/2);
		    var textFinalValue = parseFloat(pct).toFixed(2);
		    var textStartValue = config.valueCountUp?config.minValue:textFinalValue;
		    var percentText = config.displayPercent?"%":"";
		    var circleThickness = config.circleThickness * config.radius;
		    var circleFillGap = config.circleFillGap * config.radius;
		    var fillCircleMargin = circleThickness + circleFillGap;
		    var fillCircleRadius = config.radius - fillCircleMargin;
		    ...
		    var gaugeCircleY = d3.scale.linear().range([0,config.radius]).domain([0,config.radius]);
		    ...
		    var gaugeGroup = gauge.attr('transform','translate(0, 0)');
		    ...
		    var gaugeCircleArc = d3.svg.arc()
		        ...
		        .innerRadius(gaugeCircleY(config.radius-circleThickness));
...
*/
		    ...
		    var text1 = gaugeGroup.append("text")
		        ...
		        .attr('transform','translate('+config.radius+','+textRiseScaleY(config.textVertPosition)+')');
